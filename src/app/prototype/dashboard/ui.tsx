...
export function AppWindow({
  ...
  /** false holds the page still, as a chapter shows it; the review pages keep scrolling */
  scroll?: boolean;
  /** the window's box; the site's chapters size it to their frame */
  size?: string;
  /** false closes the Core's docked column (a demo can float it instead) */
  coreDocked?: boolean;
  /** a demo's own Core column in the docked place, instead of the empty one */
  coreColumn?: ReactNode;
  /** layered over the whole window, e.g. the Core lifted over the page */
  overlay?: ReactNode;
  theme: Theme;
  ...
  return (
    <div
      className={`relative flex ${size} overflow-hidden rounded-xl bg-chrome font-sans text-[13px] text-ink antialiased`}
      style={vars}
    >
      ...
      <div className="flex min-w-0 flex-1 flex-col border-l border-ink/8 bg-parchment">
        ...
        <div className={`min-h-0 flex-1 ${scroll ? "overflow-y-auto" : "overflow-hidden"}`}>{main}</div>
      </div>
      {coreDocked && (coreColumn ?? <CorePanel />)}
      {overlay}
    </div>
  );
}
