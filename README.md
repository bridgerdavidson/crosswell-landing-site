...
- **Team section (removed 2026-09-16).** The portrait cards, the bios, and the headshots (with the `turtleneck` easter egg) are gone from `/team`, which keeps the values and the closing call. The history is in git before commit "chore: the team section comes out".

## Page order (2026-09-12 general-market redesign)

Three pages. `/`: Nav, Hero, what we do (the statement, the two figures, the stack), the product run (five chapters: Today, Ask the Core, Agenda, Agents on the dark band, Custom), Who it's for, How we start, Final CTA (dark), Footer. `/team`: Values, Final CTA, Footer (the bios and headshots came out in September 2026). `/insights`: the blog's held slot, Footer. The brain map and the value ledger are parked in the repo for a future page on the Core.
