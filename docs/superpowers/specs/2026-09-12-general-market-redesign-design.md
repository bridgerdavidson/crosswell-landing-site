...
- Chapter 01 renders the Home window, display only (its old entrance sequence and Replay went with the old markup), lit after Linear's treatment rather than dimming the whole frame and lifting one element: the window is cropped to its top 600 and dissolves into the page on an L, 240 up from the bottom edge and 360 in from the right edge, multiplied where they meet so the corner turns on a curve (`.window-light`). The greeting, the numbers, the draw approval, and the morning's schedule stay at full strength.
