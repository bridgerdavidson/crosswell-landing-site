...
- `WindowFrame`: the whole window at 640 tall; from 1376 of viewport it takes the frame's width, never over 1440, and shows whole. On a laptop between lg and 1376 the column is narrower than the window, so the whole window scales down as one to fit it (0.93 at 1280, 0.73 at 1024): a laptop sees the whole dashboard a little smaller, never a dashboard cut by the frame's edge (Linear's product shots do the same at laptop widths, and the Core's stage already did). Below lg the frame shows a crop of the same window (`phone`, a rectangle in the window's own pixels): the piece the chapter's claim is about, at 1:1 where the frame holds it (a tablet) and scaled down as one where it does not (a phone), never re-flowed. The window keeps its shape and layout; the frame moves over it like a camera. And the window is never cut by an edge of the frame's own, the rule Linear's phone product shots keep: the crop starts at the window's top-left corner so its own corners show, the crop holds the window's whole height so its bottom edge shows, and the window runs off one side of the screen (the frame starts 12 past the words and runs to the viewport's edge), fading as it goes over the crop's `fade` (`window-bleed-right` / `window-bleed-left`, 96 unless set). No bottom fade: the bottom of the dashboard is always in view. The only edges a visitor sees are the screen's and the window's. A chapter with no crop gets the old fallback, the whole window scaled to the frame. Chapter 01's crop is the window's top-left 690 by 640, fade 72: rail, greeting, the morning's numbers, the whole list with its Approve and Review buttons, and the day's column starting at the screen's edge, at about 0.55 on a phone (the whole first screen in a frame about 350 tall, Linear's proportion), so a phone reads the page's structure more than its sentences. The three morning items carry a one-line `phone` body for it. Chapter 03's crop is the window's top-left 850 by 800, fade 40: rail, the date, the morning and early afternoon of the day, and the whole of the to-do and team column, the empty Core column off the screen. Below lg the day is 400 wide with a 24 gap and a 264 aside (from lg the day takes what the 280 aside leaves it) so the whole aside sits inside the crop; the next call's countdown and the day's arrows are hidden there so the date line and the standup's row still fit. The window keeps its 800 (its desktop bottom-and-right dissolve, `agenda-fade`, is off below lg, and there is no bottom fade of any kind on a phone), so the whole day to 6 pm and all four of the team sit above the window's own bottom edge, in a frame about 355 tall. Chapter 05's crop is the Core's column whole (x 708 to 1308 by 640, fade 40 on the left): the place where the work is named and the draft comes back for a yes, with the roster's last columns running off the left of the screen so the window reads as continuing; the window is 640 tall below lg (the desktop's crescent, `agents-crescent`, is off there), tall enough for the whole email draft to stand in the thread when it arrives, about 0.63 on a phone (a frame about 400 tall), the column at 1:1 on a tablet. Its words run a size up below lg like chapter 02's, Send and Approve take tap room, and its thread is a scrolling chat whose keep-in-view is set once (the column re-renders on every tick of the roster's loop). Chapter 06's crop is chapter 01's (the window's top-left 690 by 640, fade 72): the rail where the mark changes, the top bar where the action changes, the greeting, the numbers, and the left list, the right column starting at the screen's edge; the window is 640 tall below lg like chapter 01's, every page ending in the window's own fade, so the frame is about 350. The morph's measuring divides by the frame's scale, so the shells land where the sections do at any size.
...
- Stage (`src/components/product/core/Core.tsx`): the window takes the frame's width (1280 to 1440) less 12 of room on the right, with 24 above and below, so the lifted Core never crops; a narrower frame scales the whole stage down. Below lg the window is 600 tall instead of 800, ending just under the last funded deal, and the stage shows a phone crop under WindowFrame's rules (x 528 to 1308, y -24 to 672, fade 20 on the left, the room below for the whole of the lifted Core's shadow; the board's columns end 16 inside the window there, so its bottom edge reads as its own): from a sliver of the Underwriting column, so the board runs off the left of the screen and fades as it goes (the short fade stops at the lit card's edge, which stays crisp), across Docs out and Funded to the Core's column whole, with the lifted Core's overhang above and its shadow below; half scale on a phone (a frame about 330 tall), 1:1 on a tablet. The desktop's bottom-left fall-off (`core-falloff`) is off below lg, so the whole window shows to its bottom edge; the wash stays. Since the crop is half scale, the Core's words run a size up below lg (the lifted Core at 16, its question pills 16 in taller rounds), same layout; every other control keeps its desktop size (bigger ones looked out of place beside their neighbours) and takes tap room instead, an invisible 16 above and below and 8 to each side (`tap-room`). The tap flow is unchanged: Ask the Core, the questions, the drafted check-in and its Send all sit inside the crop. The lifted Core's thread is a scrolling chat: it keeps its newest line in view as it grows and scrolls back once it is longer than the column, on desktop and phone alike.
...
## 23. How we start, as three steps (hand-tuning pass, 2026-09-15)

The section's three cards (the audit, the Core install, the Core plus the custom layer) were the page's last fragment-era material, and they cut the offer in the wrong places. First drawn as two rows (the audit, onboarding) with the thirty-minute call in the band's aside, which read as a note about the audit rather than the step it is. The offer is three steps: the first call, the audit, and build and onboarding. No install vocabulary anywhere: how it works is that we help the team get onboarded, so the third step says build, then onboard, never install or provision.
...
- **`/contact` is whole from day one**, since it needs no writing: the one way in as the call button, and the address. Indexed and in the sitemap. The scheduler replaces the mailto link when it is picked.
...
## 25. One call to action (hand-tuning pass, 2026-09-15)

The hero and the closing band asked twice: "Start with the audit" and "Set up a call". The work is consulting, custom and personal, with nothing to sign up for, and How we start now says its first step is the call, so a button that starts with the audit skipped step one and read awkwardly beside the other. Every ask on the site is now the one call: the hero's button, the closing band's, the nav's, and the contact page's ("It starts with a call."). The audit mailto is gone from lib/site.ts. The closing band's line says where it starts: "The Core is built alongside the firms it serves. It starts with a thirty-minute call." The button alone read as uncomfortable, so beside it sits a quiet text link for the visitor who is not ready, "See how it works" with a down arrow that nudges on hover, scrolling to the what-we-do statement under the nav without writing a hash (`components/SeeHowItWorks`). Chosen over a line under a larger button ("Thirty minutes. We tell you straight whether it's worth going further."), which did more for the person deciding but sat less comfortably in the composition; a right arrow on the button itself was rejected, since it promises a page and delivers a mail client.
