...
The hero and the closing band asked twice: "Start with the audit" and "Set up a call". The work is consulting, custom and personal, with nothing to sign up for, and How we start now says its first step is the call, so a button that starts with the audit skipped step one and read awkwardly beside the other. Every ask on the site is now the one call: the hero's button, the closing band's, the nav's, and the contact page's ("It starts with a call."). The audit mailto is gone from lib/site.ts. The closing band's line says where it starts: "The Core is built alongside the firms it serves. It starts with a thirty-minute call." Held in reserve: a quiet text link in the hero for the visitor who is not ready, pointing down the page, to add only if the hero reads lopsided with one button.
