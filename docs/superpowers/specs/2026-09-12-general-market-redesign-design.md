...
- **The morph.** Every block in all three pages carries the same `data-morph` skeleton (greet, sub, hero-n, left-n, right-n), so a block can travel from where its counterpart stood in the last business to where it belongs in this one. Blocks translate but never scale: a list row stretched into a timeline bar smears the text inside it. A block travels the whole 0.85s and fades up in the back half, once the outgoing page (a clone, left in place and faded out over its first third) has cleared, so two pages are never legible at once. Blocks arrive in reading order, 22ms apart, so the page rebuilds down the screen. What has no counterpart rises in last, which is the part that reads as built for them.
