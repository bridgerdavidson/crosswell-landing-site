...
- `WindowFrame`: the whole window at 640 tall; from lg it takes the frame's width, never under 1280 or over 1440, so from 1440 up it shows whole and below that its right side crops. Below lg the frame shows a crop of the same window (`phone`, a rectangle in the window's own pixels): the piece the chapter's claim is about, at 1:1 where the frame holds it (a tablet) and scaled down as one where it does not (a phone), never re-flowed. The window keeps its shape and layout; the frame moves over it like a camera, the way Linear's phone chapters show one panel at real size rather than the whole app at a quarter. The frame runs 12 past the words on each side there. A chapter with no crop gets the old fallback, the whole window scaled to the frame. Chapter 01's crop is the page's left side, x 69 to 580 by 640 tall: greeting, the morning's numbers, and the list with its Approve and Review buttons; the rail and the Core's column wait outside.
