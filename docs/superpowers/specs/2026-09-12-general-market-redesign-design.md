...
## 23. How we start, as two steps (hand-tuning pass, 2026-09-15)

The section's three cards (the audit, the Core install, the Core plus the custom layer) were the page's last fragment-era material, and one step more than the offer has. The offer is two: the audit, where we come in, ask questions, learn the business and how its work moves, and design the system from that; and onboarding, where the Core is built, installed, and handed over running with the team on it, the custom layer following.

- **Two rows on the band's grid** (chosen over two side-by-side columns set like the values, which read as two things rather than first this then that, and over a two-stop timeline, a diagram with little to diagram). The title band stays as it was, the first-call sentence and the audit button on its right. Under it, hairline rows: each leads with its drawing at 96, then the step's name in serif with its duration as a fern caption, then what happens, set in the lede column so it sits on the band's words above.
- **Two drawings** (`components/StartArt.tsx`), in the stack's hand: the same ink strokes, ivory cards, and fern accent as the what-we-do stack, so the page has one drawing language. The audit: three things the company knows lying where they are, a dashed route walked between them, and the one place along it ringed in fern, where the work drops what it knew, the route drawn in fern from there. Onboarding: the stack itself, small and side on, the Core plate with its points joined, the work plate with three tiles, the dashboard on top with its one green light, ringed: it is on.
