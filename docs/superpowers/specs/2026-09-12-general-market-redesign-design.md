...
The section's three cards (the audit, the Core install, the Core plus the custom layer) were the page's last fragment-era material, and they cut the offer in the wrong places. First drawn as two rows (the audit, onboarding) with the thirty-minute call in the band's aside, which read as a note about the audit rather than the step it is. The offer is three steps: the first call, the audit, and build and setup.
