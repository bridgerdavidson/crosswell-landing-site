...
- Chapter 01 renders the Home window, display only (its old entrance sequence and Replay went with the old markup), after Linear's treatment rather than dimming the whole frame and lifting one element: the window is cropped to its top 600, clear across its top, and dissolves into the page over its bottom 240 (`.window-light`). An L-shaped fade (bottom and right) was tried first and dropped: with the Core's column empty, the right side read as the picture ending. The Core's column holds one exchange ("Which draws are due this week?", its answer, and its sources, from `today.ask`), so the first dashboard a visitor sees shows the Core is in it; chapter 03 keeps its own question.
- `WindowFrame`: from lg the window takes the frame's width, never under 1280 or over 1440, and the frame shows its top 600, so from 1440 up the window's full width shows and below that its right side crops; below lg the 1280 by 600 crop scales down whole (a phone pass is still owed).
