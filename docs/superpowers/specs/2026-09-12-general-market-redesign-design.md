...
- **The morph, in three beats.** Every block in all three pages carries the same `data-morph` skeleton (greet, sub, hero-n, left-n, right-n). On the change, each block's box becomes a bare shell (a rounded 7% ink block drawn in a layer over the page): the page fades out under them (0.22s), the shells travel and resize into this business's layout (0.58s, in reading order 12ms apart, carrying the colours and the chrome with them), and the new page fills them in (0.34s, its blocks 15ms apart). A shell with no counterpart in the new layout goes as the others move; a section this business alone has arrives late, so the shape is whole before it fills. The first pass moved the content itself and read as a cut rather than a morph: blocks cannot be scaled (a list row stretched into a timeline bar smears its text), so the bare shells are what carries the change of shape.
